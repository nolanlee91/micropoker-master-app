...
// Deterministic PRNG (mulberry32) seeded from the exact inputs, so recalculating
// the SAME spot always yields the SAME equities — no run-to-run jitter. (Monte
// Carlo is still an approximation, just a stable one.)
...
// Seeded so the same input is stable across recalculations.
function simulate(heroHole, villains, board, iters = 20000) {
  const used = new Set([...heroHole, ...villains.flat(), ...board])
  const avail = DECK.filter(c => !used.has(c))
  const need = 5 - board.length
  const scores = new Array(1 + villains.length).fill(0)
  const rng = makeRng(seedFrom([...heroHole, '|', ...villains.flat(), '|', ...board]))
